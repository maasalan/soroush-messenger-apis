...
import android.support.v4.p031b.p032a.C0430b;
...
*/
        ...
        while (true) {
            ...
            if (VERSION.SDK_INT >= 17 || !(drawable instanceof LayerDrawable)) {
                if (!(drawable instanceof DrawableContainer)) {
                    if (!(drawable instanceof C0430b)) {
                        if (!(drawable instanceof C0687a)) {
                            if (!(drawable instanceof ScaleDrawable)) {
                                break;
                            }
                            drawable = ((ScaleDrawable) drawable).getDrawable();
                        } else {
                            drawable = ((C0687a) drawable).getWrappedDrawable();
                        }
                    } else {
    ...
    private static void fixVectorDrawableTinting(Drawable drawable) {
        ...
        if (state != null) {
            if (state.length != 0) {
                iArr = ThemeUtils.EMPTY_STATE_SET;
                drawable.setState(iArr);
                drawable.setState(state);
            }
        }
        iArr = ThemeUtils.CHECKED_STATE_SET;
        drawable.setState(iArr);
        drawable.setState(state);
    }
...
*/
    ...
    L_0x0004:
        r10 = android.support.v4.p031b.p032a.C0429a.m897h(r10);	 Catch:{ Exception -> 0x0098 }
